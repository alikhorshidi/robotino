...
// (توضیح استاد): این فرکانس، سرعت چرخش موتورها را تعیین می‌کند.
...
// (توضیح استاد): ما از نصف رزولوشن (128) برای ایجاد موج مربعی 50% استفاده می‌کنیم.
...
void setup() {
  ...
  // (توضیح استاد): درایورهای TB6600 با LOW شدن پین EN فعال می‌شوند.
  ...
  // (توضیح استاد): این بخش، قلب تپنده سیستم است.
  ...
  // (توضیح استاد): در اینجا، هر کانال سخت‌افزاری را به پین فیزیکی پالس متصل می‌کنیم.
...
// (توضیح استاد): این تابع با ارسال یک موج مربعی 50% (مقدار 128) به هر چهار کانال،
...
// (توضیح استاد): این تابع با ارسال Duty Cycle صفر، تولید پالس را متوقف می‌کند.
...
```cpp
// (توضیح استاد): پیاده‌سازی منطق جدول ۱ برای حرکت به جلو
void moveForward() {
...
// (توضیح استاد): پیاده‌سازی منطق جدول ۱ برای حرکت به عقب
void moveBackward() {
...
// (توضیح استاد): پیاده‌سازی منطق جدول ۱ برای حرکت جانبی به چپ
void strafeLeft() {
...
// (توضیح استاد): پیاده‌سازی منطق جدول ۱ برای حرکت جانبی به راست
void strafeRight() {
...
// (توضیح استاد): پیاده‌سازی منطق جدول ۱ برای چرخش ساعتگرد
...
// (توضیح استاد): پیاده‌سازی منطق جدول ۱ برای چرخش پادساعتگرد
 ...
 *
 * (توضیح استاد): این کد را به دقت مطالعه کنید. به نحوه تعریف پارامترها،
...
// (توضیح استاد): هر پین پالس باید به یک کانال سخت‌افزاری LEDC مجزا متصل شود.
...
void setup() {
  ...
  // (توضیح استاد): در این 4 خط، ما تایمرهای سخت‌افزاری را پیکربندی می‌کنیم.
  ...
  // (توضیح استاد): در این 4 خط، ما هر کانال سخت‌افزاری را به پین فیزیکی
